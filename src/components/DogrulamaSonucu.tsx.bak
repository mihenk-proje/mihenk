...
export function DogrulamaSonucu({ sonuc, onClose }: DogrulamaSonucuProps) {
  ...
  useEffect(() => {
    if (sonuc) {
      setShowLine(false)
      // Biraz bekle ve çizgiyi çiz (animasyon için)
  ...
  // Skora göre çizgi kalınlığı ve doygunluğu (bilgi taşıyan öğe)
  const lineStrokeWidth = Math.max(1, Math.min(8, sonuc.skor / 12)) // 1px - 8px arası
  const lineOpacity = Math.max(0.3, sonuc.skor / 100) // %30 - %100 arası
    ...
    <div className="fixed bottom-0 left-0 right-0 z-50 p-4 sm:p-6 pointer-events-none flex justify-center">
      ...
      >
        ...
        <div className="p-5">
          ...
          {!isSuccess && (
            <button className="w-full mt-2 py-2.5 bg-page hover:bg-page/80 border border-primary text-primary font-medium rounded-lg transition-colors">
